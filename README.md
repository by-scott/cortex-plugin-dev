...
The packer auto-resolves the native library from `target/release/` based on `manifest.toml` and writes `cortex-plugin-dev-v1.6.1-linux-amd64.cpx`. Packaged installs require a valid Cortex package signature; `--yes` should only be used after the publisher key fingerprint has been reviewed.
